...
(define (game-starting-message)
  (printf "~a~a~a~%"
          (ansi #:text 'green) "Game is starting." reset))
...
(define (run args)
  ...
  (define port (if (non-empty-string? maybe-port) maybe-port (hash-ref args 'port)))
  ...
  (define module-str (string-append "games/" game-name-downcase "/" game-name-downcase ".rkt"))
  ...
  (define player-name (hash-ref args 'player-name))
